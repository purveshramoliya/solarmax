...
{strip}
    ...
    {assign var="paymentno" value="paymentno"|cat:$row_no}
	...
	<td>
		<i class="icon-trash deleteRow cursorPointer" id="icon-trash" title="{vtranslate('LBL_DELETE',$MODULE)}"></i>
		&nbsp;<a><img src="{vimage_path('drag.png')}" border="0" title="{vtranslate('LBL_DRAG',$MODULE)}"/></a>
		<input type="hidden" class="rowNumber" value="{$row_no}" />
	</td>
	 {if $MODULE neq 'PayuMoney'}
	<td>
	...
	{if $MODULE eq 'PayuMoney'}
	<td>
	<select id="{$paymentmode}" name="{$paymentmode}"/ style="width:125px;" >
		...
		<td>
		<input id="{$paymentno}" name="{$paymentno}" type="text"  value="{$data.$paymentno}" style="width:120px;">
	</td>
	<td>
		<textarea id="{$bankdetails}" name="{$bankdetails}" class="lineItemCommentBox">{$data.$bankdetails}</textarea>
	</td>

   <td>
		<input id="{$paymentdate}" name="{$paymentdate}" type="date" value="{$data.$paymentdate}" style="width:130px;">
	</td>
 ...
 {if $MODULE neq 'PayuMoney'}
	...
	{if	$MODULE eq 'PayuMoney'}
		<div  style="display:none;" id="discountTotal{$row_no}" align="right" class="discountTotal">{if $data.$discountTotal}{$data.$discountTotal}{else}0{/if}</div>
		<div style="display:none;" id="totalAfterDiscount{$row_no}" align="right" class="totalAfterDiscount">{if $data.$totalAfterDiscount}{$data.$totalAfterDiscount}{else}0{/if}</div>
		<div style="display:none;" id="taxTotal{$row_no}" align="right" class="productTaxTotal {if $IS_GROUP_TAX_TYPE}hide{/if}">{if $data.$taxTotal}{$data.$taxTotal}{else}0{/if}</div>
