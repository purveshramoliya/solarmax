...
Vtiger_Edit_Js("Inventory_Edit_Js",{
	...
	updateLineItemsElementWithSequenceNumber : function(lineItemRow,expectedSequenceNumber , currentSequenceNumber){
		...
		var idFields = new Array('productName','subproduct_ids','hdnProductId',
							   'comment','paymentmode','paymentno','bankdetails','paymentdate','qty','listPrice','discount_type','discount_percentage',
    ...
    registerEvents: function(){
		...
		this.registerFieldDependencyEvent();
    },
	registerFieldDependencyEvent: function (container) {
        var Tasktype = $("select[name='system'] option:selected").val();
        if (Tasktype == 'New') {
            jQuery('#Quotes_editView_fieldName_cf_1043').removeClass('hide');
			jQuery('#Quotes_editView_fieldName_cf_1045').removeClass('hide');
			jQuery('#Quotes_editView_fieldName_cf_1047').removeClass('hide');
        }
        jQuery("select[name='system']").on('change', function(e){
            var currentTarget = jQuery(e.currentTarget);
            var subTasktype = currentTarget.val();
            if (subTasktype == 'New') {
                jQuery('#Quotes_editView_fieldName_cf_1043').addClass('hide');
				jQuery('#Quotes_editView_fieldName_cf_1045').addClass('hide');
				jQuery('#Quotes_editView_fieldName_cf_1047').addClass('hide');
            } else {
				jQuery('#Quotes_editView_fieldName_cf_1043').removeClass('hide');
				jQuery('#Quotes_editView_fieldName_cf_1045').removeClass('hide');
				jQuery('#Quotes_editView_fieldName_cf_1047').removeClass('hide');
