...
class PurchaseOrder extends CRMEntity {
	...
	function save_module($module)
	{
		...
		$this->insertIntoAttachment($this->id,'PurchaseOrder');
	} 
	...
	function createRecurringInvoiceFromSO(){
		...
		for($j=0; $j<$no_of_products; $j++) {
			...
			if(count($col_value) > 0) {
				...
				updateStk($prod_id,$qty,'',array(),'PurchaseOrder');
			}
		}
		...
		$updatequery  = " UPDATE vtiger_purchaseorder SET ";
		$updateparams = array();
		...
		if (count($updatecols) > 0) {
			...
			$updatequery .= " WHERE purchaseorderid=?";
	...
	function insertIntoAttachment($id,$module)
	{
		...
		if($module == 'PurchaseOrder' && $_REQUEST['del_file_list'] != '')
		{
