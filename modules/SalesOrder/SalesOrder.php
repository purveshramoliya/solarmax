	...
	function save_module($module)
	{
		...
		if(isset($this->_recurring_mode) && $this->_recurring_mode == 'recurringinvoice_from_so' && isset($this->_salesorderid) && $this->_salesorderid!='') {
			...
			if($_REQUEST['action'] != 'SalesOrderAjax' && $_REQUEST['ajxaction'] != 'DETAILVIEW'
				...
				if (!empty($record)) {
					$invoiceid = '';
					$sqlState = "SELECT purchaseorderid FROM `vtiger_purchaseordercf` " .
					" inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_purchaseordercf.purchaseorderid " .
					" where vtiger_crmentity.deleted = 0  and cf_932 = ? ";

					$res = $adb->pquery($sqlState, array($record));
					$noOfRows = $adb->num_rows($res);
					for ($index = 0; $index < $noOfRows; ++$index) {
						$invoiceid = $adb->query_result($res, $index, 'purchaseorderid');
					}
					if (!empty($invoiceid)) {
					...
					$invoiceid = '';
					$sqlState = "SELECT invoiceid FROM `vtiger_invoice` " .
	...
	function createRecurringInvoiceFromSO(){
		...
		for($j=0; $j<$no_of_products; $j++) {
			...
			if(count($col_value) > 0) {
				...
				updateStk($prod_id,$qty,'',array(),'SalesOrder');
			}
		}
		...
		$updatequery  = " UPDATE vtiger_salesorder SET ";
		$updateparams = array();
		...
		if (count($updatecols) > 0) {
			...
			$updatequery .= " WHERE salesorderid=?";
			array_push($updateparams, $this->id);
			
			$result  = $adb->pquery($updatequery, $updateparams);
