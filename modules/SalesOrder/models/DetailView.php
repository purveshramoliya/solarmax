...
class SalesOrder_DetailView_Model extends Inventory_DetailView_Model {
	...
	public function getDetailViewLinks($linkParams) {
		...
		if($currentUserModel->hasModuleActionPermission($invoiceModuleModel->getId(), 'EditView')) {
            ...
            if($recordModel->get('sostatus') == 'Approved'){
		...
		$custpurchaseOrderModuleModel = Vtiger_Module_Model::getInstance('Custpurchase');
		if($currentUserModel->hasModuleActionPermission($custpurchaseOrderModuleModel->getId(), 'EditView')) {
			$basicActionLink = array(
					'linktype' => 'DETAILVIEW',
					'linklabel' => vtranslate('LBL_CREATE').' '.vtranslate($custpurchaseOrderModuleModel->getSingularLabelKey(), 'Custpurchase'),
					'linkurl' => $recordModel->getCreateCustPurchaseOrderUrl(),
					'linkicon' => ''
			);
			$linkModelList['DETAILVIEW'][] = Vtiger_Link_Model::getInstanceFromValues($basicActionLink);
		} 
