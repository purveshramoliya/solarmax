	...
	function save_module()
	{
	  global $adb;
	   ...
	   $total=$_REQUEST['total'];
	    ...
	    //  if($record != '')
	    //  {
	    //      $adb->pquery("update vtiger_salesorder set adjustment='$adjustment',total='$total' where quoteid=$record");
	    //  }
		...
		if($_REQUEST['action'] != 'QuotesAjax' && $_REQUEST['ajxaction'] != 'DETAILVIEW'
			...
			if(!empty($record)){
				$invoiceid = ''; 
				$sqlState = "SELECT invoiceid FROM `vtiger_invoicecf` " .
				" inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_invoicecf.invoiceid " .
				" where vtiger_crmentity.deleted = 0  and cf_928 = ? ";
				$res = $adb->pquery($sqlState, array($record));
				$noOfRows = $adb->num_rows($res);
				for($index = 0; $index < $noOfRows; ++$index) {
					$invoiceid = $adb->query_result($res,$index,'invoiceid');
				}
				...
				if(!empty($invoiceid)){
					...
					$recordModel = Vtiger_Record_Model::getInstanceById($recdId, $moduleName);
					$recordModel->set('id', $recdId);
					...
					$modMeta  =  Vtiger_Record_Model::getInstanceById($recdId,'Invoice');
					$ad->handleEvent('vtiger.entity.aftersave' , $modMeta , "True");
				}

				$invoiceid = ''; 
				$sqlState = "SELECT salesorderid FROM `vtiger_salesorder` " .
				" inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_salesorder.salesorderid " .
				" where vtiger_crmentity.deleted = 0  and quoteid = ? ";
				$res = $adb->pquery($sqlState, array($record));
				$noOfRows = $adb->num_rows($res);
				for($index = 0; $index < $noOfRows; ++$index) {
					$invoiceid = $adb->query_result($res,$index,'salesorderid');
				}
				if(!empty($invoiceid)){
					$amgutka  = new stdClass;
					$amgutka->id  =  $invoiceid;
					$amgutka->mode  =  'edit';
					$amgutka->table_name  =  "vtiger_salesorder";
					$amgutka->table_index  =  'salesorderid';
					saveInventoryProductDetails($amgutka, 'SalesOrder');
				}
			}
		}
