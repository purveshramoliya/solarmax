...
class Vtiger_RelationListView_Model extends Vtiger_Base_Model {
	...
	public function getCreateViewUrl(){
		...
		if($parentmodule=='Invoice' && $relatedmodule=='PayuMoney')
		{
		$cid=$parentRecordModule->get('contact_id');
		$iid=$parentRecordModule->get('invoice_no');
		$date=$parentRecordModule->get('duedate');
		$ddate=date("d-m-Y", strtotime($date));
		$bs=$parentRecordModule->get('bill_street');
		$bc=$parentRecordModule->get('bill_city');	
		$bss=$parentRecordModule->get('bill_state');
		$bcc=$parentRecordModule->get('bill_code');
		...
		$createViewUrl = $relatedModel->getCreateRecordUrl().'&sourceModule='.$parentModule->get('name').
								'&sourceRecord='.$parentRecordModule->getId().'&relationOperation=true&contact_id='.$cid.'&cf_896='.$iid.'&duedate='.$ddate.'&bill_street='.$bs.'&bill_city='.$bc.'&bill_state='.$bss.'&bill_code='.$bcc;
		}
else
{
		$createViewUrl = $relatedModel->getCreateRecordUrl().'&sourceModule='.$parentModule->get('name').
