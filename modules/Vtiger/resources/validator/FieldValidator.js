...
Vtiger_Base_Validator_Js("Vtiger_MaxNumber_Validator_Js",{
	invokeValidation: function(field, rules, i, options){
		var positiveNumberInstance = new Vtiger_PositiveNumber_Validator_Js();
		positiveNumberInstance.setElement(field);
		var response = positiveNumberInstance.validate();
	...
	validate: function(){
		var fieldValue = this.getFieldValue();
		if((fieldValue.toString().length < 10 ) || (fieldValue.toString().length > 11 )){
			var errorInfo = 'THis should be grater than 10 charcters and lessthan 11 charcters';
			this.setError(errorInfo);
			return false;
		}
		return true;
