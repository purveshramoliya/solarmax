...
class PDFMaker_SaveRelatedBlock_Action extends Vtiger_Action_Controller
{
    ...
    public function process(Vtiger_Request $request)
    {
        ...
        if ($this->relblockid != "") {
            ...
            $adb->pquery($sql, array($name, $block, $this->relblockid));
        } else {
            ...
            $adb->pquery($sql, array($this->relblockid, $name, $module, $secmodule, $block));
    ...
    function saveSelectedFields($selectedFields)
    {
        ...
        for ($i = 0; $i < count($selectedFields); $i++) {
            if (!empty($selectedFields[$i])) {
                ...
                $adb->pquery("INSERT INTO vtiger_pdfmaker_relblockcol (relblockid, colid, columnname) VALUES (?,?,?)",
    ...
    function saveAdvancedFilters($advancedFilter)
    {
        ...
        if (!empty($advancedFilter)) {
            ...
            $adb->pquery('DELETE FROM vtiger_pdfmaker_relblockcriteria WHERE relblockid = ?', array($this->relblockid));
            $adb->pquery('DELETE FROM vtiger_pdfmaker_relblockcriteria_g WHERE relblockid = ?', array($this->relblockid));
            ...
            foreach ($advancedFilter as $groupIndex => $groupInfo) {
                ...
                $adb->pquery("INSERT INTO vtiger_pdfmaker_relblockcriteria_g (groupid, relblockid, group_condition, condition_expression) VALUES (?,?,?,?)",
    ...
    {
        $adb = PearDatabase::getInstance();
        $adb->pquery('DELETE FROM vtiger_pdfmaker_relblocksortcol WHERE relblockid = ?', array($this->relblockid));
        $i = 0;
        foreach ($sortFields as $fieldInfo) {
            $adb->pquery('INSERT INTO vtiger_pdfmaker_relblocksortcol (sortcolid, relblockid, columnname, sortorder) VALUES (?,?,?,?)',
