    ...
    public function process(Vtiger_Request $request)
    {
        ...
        if ($request->has('templateid') && !$request->isEmpty('templateid')) {
            ...
            if ($pdftemplateResult["type"] != "") {
                ...
                if ($pdftemplateResult["type"]) {
                    ...
                    $viewer->assign("TEMPLATEBLOCKTYPE", $blocktype);
                }
            }
        ...
        if (!$is_block) {
            ...
            foreach (array("header", "footer") as $block_type) {
                ...
                $BlockList = $PDFMaker->GetListviewData("templateid", "ASC", $BRequest);
        ...
        if ($request->has("isDuplicate") && $request->get("isDuplicate") == "true") {
            ...
            $viewer->assign("DUPLICATE_TEMPLATEID", $templateid);
        } else {
...
// ******************************************   Company and User information: **********************************
        ...
        if (getTabId('ITS4YouMultiCompany') && vtlib_isModuleActive('ITS4YouMultiCompany')) {
            $MultiAcc_info = array(
                ...
                "MULTICOMPANY_COMPANYNAME" => vtranslate("LBL_COMPANY_NAME", 'ITS4YouMultiCompany'),
                "MULTICOMPANY_STREET" => vtranslate("Street", 'ITS4YouMultiCompany'),
                "MULTICOMPANY_CITY" => vtranslate("City", 'ITS4YouMultiCompany'),
                "MULTICOMPANY_CODE" => vtranslate("Code", 'ITS4YouMultiCompany'),
                "MULTICOMPANY_STATE" => vtranslate("State", 'ITS4YouMultiCompany'),
                "MULTICOMPANY_COUNTRY" => vtranslate("Country", 'ITS4YouMultiCompany'),
                "MULTICOMPANY_PHONE" => vtranslate("phone", 'ITS4YouMultiCompany'),
                "MULTICOMPANY_FAX" => vtranslate("Fax", 'ITS4YouMultiCompany'),
                "MULTICOMPANY_EMAIL" => vtranslate("email", 'ITS4YouMultiCompany'),
                "MULTICOMPANY_WEBSITE" => vtranslate("Website", 'ITS4YouMultiCompany'),
                "MULTICOMPANY_LOGO" => vtranslate("Logo", 'ITS4YouMultiCompany'),
                "MULTICOMPANY_STAMP" => vtranslate("Stamp", 'ITS4YouMultiCompany'),
                "MULTICOMPANY_BANKNAME" => vtranslate("BankName", 'ITS4YouMultiCompany'),
                "MULTICOMPANY_BANKACCOUNTNO" => vtranslate("BankAccountNo", 'ITS4YouMultiCompany'),
                "MULTICOMPANY_IBAN" => vtranslate("IBAN", 'ITS4YouMultiCompany'),
                "MULTICOMPANY_SWIFT" => vtranslate("SWIFT", 'ITS4YouMultiCompany'),
                "MULTICOMPANY_REGISTRATIONNO" => vtranslate("RegistrationNo", 'ITS4YouMultiCompany'),
                "MULTICOMPANY_VATNO" => vtranslate("VATNo", 'ITS4YouMultiCompany'),
                "MULTICOMPANY_TAXID" => vtranslate("TaxId", 'ITS4YouMultiCompany'),
                "MULTICOMPANY_ADDITIONALINFORMATIONS" => vtranslate("AdditionalInformations", 'ITS4YouMultiCompany'),
            );
            $viewer->assign("MULTICOMPANYINFORMATIONS", $MultiAcc_info);
            $viewer->assign("LBL_MULTICOMPANY", vtranslate('MultiCompany', 'ITS4YouMultiCompany'));
        }
...
//Sharing
        ...
        if (Vtiger_Module_Model::getInstance("ITS4YouMultiCompany") !== false && Vtiger_Module_Model::getInstance("ITS4YouMultiCompany")->isActive() !== false) {
            $sharing_types["mc"] = vtranslate("MULTICOMPANY_SHARE_FILTER", 'ITS4YouMultiCompany');
        }
...
// header / footer display settings
        ...
        $viewer->assign('MAX_UPLOAD_LIMIT_BYTES', (Vtiger_Util_Helper::getMaxUploadSize() * 1024 * 1024));

        $Watermark = $recordModel->getWatemarkData();
        $viewer->assign('WATERMARK', $Watermark);

        $fontawesomeclass = file_get_contents('layouts/vlayout/modules/PDFMaker/resources/FontAwesome.css');
        $viewer->assign('FONTAWESOMECLASS', $fontawesomeclass);
        $viewer->assign('FONTAWESOMEICONS', PDFMaker_FontAwesome_Helper::getIcons());

        $viewer->assign('SIGNATURE_HEIGHT', isset($pdftemplateResult['signature_height']) ? $pdftemplateResult['signature_height'] : PDFMaker_PDFContentUtils_Model::SIGNATURE_HEIGHT);
        $viewer->assign('SIGNATURE_WIDTH', isset($pdftemplateResult['signature_width']) ? $pdftemplateResult['signature_width'] : PDFMaker_PDFContentUtils_Model::SIGNATURE_WIDTH);
    ...
    public function getHeaderCss(Vtiger_Request $request)
    {
        ...
        $cssFileNames = array('~/layouts/vlayout/modules/PDFMaker/resources/FontAwesomePreview.css');
        if (vtlib_isModuleActive("ITS4YouStyles")) {
