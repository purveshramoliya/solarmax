            ...
            foreach ($data as $attid => $images) {
                if ($attid != "") {
                    ...
                    if (class_exists("Vtiger_ITS4YouImageDisplay_Action")) {
                        $image_path = 'index.php?module=' . getSalesEntityType($productid) . '&action=ITS4YouImageDisplay&record=' . $productid . '&file=' . $attid;
                    } else {
                    ...
                    $prodImg .= '<td valign="middle"><input type="radio" name="img_' . $productid . '_' . $seq . '" value="' . $attid . '"' . $checked . '/>
		                 <img align="absmiddle" src="' . $image_path . '" alt="' . $images["name"] . '" title="' . $images["name"] . '" style="max-width:50px;max-height:50px;">
		                 </td>';
                    $i++;
                }
            }
