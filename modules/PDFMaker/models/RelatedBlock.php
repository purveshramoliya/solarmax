...
class PDFMaker_RelatedBlock_Model extends Vtiger_Module_Model
{
    ...
    {
        global $is_admin, $profileGlobalPermission;
        ...
        while ($criteriatyperow = $adb->fetch_array($result)) {
            ...
            if ('vtiger_crmentity' === $fieldtablename) {
                $fieldtablename = $fieldtablename . $sec_module;
            }

            $fieldlabel1 = str_replace(" ", "_", $fieldlabel);
            $optionvalue = $fieldtablename . ":" . $fieldcolname . ":" . $sec_module . "_" . $fieldlabel1;
            $stdcriteria_list[$optionvalue] = vtranslate($fieldlabel, $sec_module);
        }

        return $stdcriteria_list;
    }

    protected $customColumnInfo = array(
    ...
    public function getColumnsListbyBlock($module, $block, $pri_module, $current_user)
    {
        ...
        for ($i = 0; $i < $noofrows; $i++) {
            ...
            $columnInfo = $this->getColumnInfo($fieldname, $module);

            if (!empty($columnInfo['table'])) {
                $fieldtablename = $columnInfo['table'];
            }
            if (!empty($columnInfo['column'])) {
                $fieldcolname = $columnInfo['column'];
            }
    ...
    {
        $db = PearDatabase::getInstance();
        $result = $db->pquery('SELECT * FROM vtiger_pdfmaker_relblocksortcol
