...
function addInventoryHistory($module, $id, $relatedname, $total, $history_fldval)
{
	...
	$history_table_array = Array(
				    ...
				    "PayuMoney"=>"vtiger_payumoneystatushistory"
				    );
	...
	if($updateInventoryProductRel_deduct_stock) {
		...
		for($index = 0;$index <$numrows;$index++) {
			...
			if($adb->num_rows($sub_prod_query)>0) {
				for($j=0;$j<$adb->num_rows($sub_prod_query);$j++) {
					$sub_prod_id = $adb->query_result($sub_prod_query,$j,"productid");
						$rsubqty=$adb->pquery("select qty_per_unit from vtiger_products where productid=$sub_prod_id");
					$subqty=$adb->query_result($rsubqty);
					$sqtyinstk= getPrdQtyInStck($sub_prod_id);
					$supd_qty = $sqtyinstk-$subqty;
...
function saveInventoryProductDetails(&$focus, $module, $update_prod_stock='false', $updateDemand='')
{
	...
	for($i=1; $i<=$tot_no_prod; $i++)
	{
        ...
        $bankdetails = vtlib_purify($_REQUEST['bankdetails'.$i]); 
        $paymentmode = vtlib_purify($_REQUEST['paymentmode'.$i]); 
        $paymentdate = vtlib_purify($_REQUEST['paymentdate'.$i]); 
        $paymentno = vtlib_purify($_REQUEST['paymentno'.$i]); 
	...
	if($module == 'PayuMoney')
		{
		$query ="insert into vtiger_inventoryproductrel(id, productid, sequence_no, quantity, bankdetails, paymentmode, paymentdate,paymentno, listprice, comment, description) values(?,?,?,?,?,?,?,?,?,?,?)";
		$qparams = array($focus->id,$prod_id,$prod_seq,$qty,$bankdetails,$paymentmode,$paymentdate,$paymentno,$listprice,$comment,$description);
		$adb->pquery($query,$qparams);
	 }
...
function getInventoryTaxType($module, $id)
{
	...
	$inv_table_array = Array('Custpurchase' => 'vtiger_custpurchase','PurchaseOrder'=>'vtiger_purchaseorder','SalesOrder'=>'vtiger_salesorder','Quotes'=>'vtiger_quotes','Invoice'=>'vtiger_invoice','PayuMoney'=>'vtiger_payumoney');
	$inv_id_array = Array('Custpurchase' => 'custpurchaseid','PurchaseOrder'=>'purchaseorderid','SalesOrder'=>'salesorderid','Quotes'=>'quoteid','Invoice'=>'invoiceid','PayuMoney'=>'payumoneyid');
